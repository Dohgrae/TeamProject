...
function buildUserMessage(answers: string[]): string {
  return WORK_INTERVIEW_QUESTIONS.map((q, i) => `Q${i + 1}. ${q}\nA${i + 1}. ${answers[i] ?? ""}`).join("\n\n");
}
...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(answers) || answers.length !== WORK_INTERVIEW_QUESTIONS.length) {
  ...
  try {
    ...
    const parsed = textBlock ? (JSON.parse(textBlock.text) as { keywords: string[] }) : { keywords: [] };
